...
class ResNetLayer(nn.Module):
    def __init__(self, in_channels, out_channels, scale='none', kernel_size=3, stride=1):
        ...
        self.identity_proccess = in_channels != out_channels or stride > 1 or scale == 'up'
        self.scale = scale
        ...
        if self.identity_proccess:
            if scale == 'down':
                ...
                self.skip_connection = nn.Sequential(
                    nn.ConvTranspose2d(in_channels, out_channels, kernel_size=1, stride=2, padding=0, bias=False),
                    nn.BatchNorm2d(out_channels)
                )
